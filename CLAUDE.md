...
- **Linting (all Node projects):** ESLint 9 **flat config** (`eslint.config.mjs` per project) + **typescript-eslint 8** (the unified `typescript-eslint` package via `tseslint.config(...)`), `eslint-plugin-prettier` (prettier runs as a lint rule, so `npm run lint:fix` reformats), and `eslint-plugin-unused-imports` (dead imports are an auto-fixable error; unused locals error in services, warn in `frontend`). `no-explicit-any`, `no-non-null-asserted-optional-chain`, `no-prototype-builtins`, and `no-useless-catch` are **warnings** (too many / too semantic to fix mechanically); `frontend` additionally warns `no-empty-object-type`, `no-useless-escape`, and `no-constant-binary-expression`. Each config is self-contained (no shared root file — that breaks `typescript-eslint` module resolution). Activate the lint pre-commit hook once per clone: `git config core.hooksPath .githooks`.
