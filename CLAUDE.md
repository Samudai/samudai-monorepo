...
- **TypeScript 7 (all Node/TS projects):** the native Go `tsc` ships no JS compiler API, so every `package.json` aliases it via `"@typescript/native": "npm:typescript@^7.0.2"` (the real TS7 binary, what `npm run build`'s `tsc -p .` actually runs) alongside `"typescript": "npm:@typescript/typescript6@^6.0.2"` (re-exports the TS 6.0 API so typescript-eslint's `<6.1.0` peer range and other API-dependent tooling keep working). `tsconfig.json`s use `module`/`moduleResolution: nodenext` (backend/bots) or `bundler` (frontend, unchanged) and explicit `"types": ["node"]` (TS7 defaults `types` to `[]`). Backend/bot `dev` scripts run on **tsx** (`tsx watch src/index.ts`), not ts-node-dev — ts-node-dev depends on the same missing TS7 API.
- **Linting (all Node projects):** ESLint 10 **flat config** (`eslint.config.mjs` per project) composed with ESLint's official **`defineConfig` / `globalIgnores`** helpers (from `eslint/config`), pulling in **typescript-eslint 8** (the unified `typescript-eslint` package) via the config object's `extends`, plus `eslint-plugin-prettier` (prettier runs as a lint rule, so `npm run lint:fix` reformats) and `eslint-plugin-unused-imports` (dead imports are an auto-fixable error; unused vars+args error in services, warn in `frontend`). In the **services/bots** `no-require-imports`, `no-useless-catch`, and unused vars/args are **errors**; `no-explicit-any`, `no-non-null-asserted-optional-chain`, and `no-prototype-builtins` are **warnings**. `frontend` keeps the lighter set (those plus `no-useless-catch` as warnings, and additionally warns `no-empty-object-type`, `no-useless-escape`, `no-constant-binary-expression`; unused locals warn) and pins `settings.react.version` explicitly rather than `'detect'` (ESLint 10's flat-config rule context dropped `getFilename()`, which `eslint-plugin-react`'s version-detection relies on). Each config is self-contained (no shared root file — that breaks `typescript-eslint` module resolution). Activate the lint pre-commit hook once per clone: `git config core.hooksPath .githooks`.
- **Frontend dependency ceiling:** `wagmi` and `viem` are held at latest **2.x** (not bumped to any 3.x) because `@rainbow-me/rainbowkit` peers on `wagmi ^2.9.0` / `viem 2.x` and has no 3.x-compatible release — check RainbowKit's peer range before bumping either.
