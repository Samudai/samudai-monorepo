    ...
    try {
      ...
      for (const action of actions) {
        if (action === "likes") {
          ...
          await this.ActivityQuery.addNewRetweets(
            twitter_userId,
            twitter_username,
            point_id,
            tweet_id,
          ...
          await this.ActivityQuery.addNewMention(
            twitter_userId,
            twitter_username,
            point_id,
          );
          ...
          await this.ActivityQuery.addNewFollowers(
            twitter_userId,
            twitter_username,
            point_id,
          );
          ...
          await this.ActivityQuery.addNewHashtags(
            twitter_userId,
            twitter_username,
            point_id,
            hashtag,
      ...
      res.status(200).json({
        message: `New twitter activity added successfully for ${twitter_username}`,
      });
    } catch (err: any) {
      if (err.response) {
        return res.status(err.response.status).send({
          message: "Error adding twitter activity",
