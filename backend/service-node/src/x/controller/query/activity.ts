  ...
  callTwitterApi = async (
    ...
    try {
      ...
      if (error?.response?.status === 401) {
        const result = await this.updateAccessAndRefreshToken(
          point_id,
          refresh_token,
        );
        return {
  ...
  updateAccessAndRefreshToken = (
    ...
    return new Promise((resolve, reject) => {
      ...
      for (const property in details) {
        const encodedKey = encodeURIComponent(property as keyof typeof details);
      ...
      axios
        ...
        .then(async (data) => {
          try {
            await axios.post(
              `${process.env.SERVICE_POINT}/twitter/updatetokens`,
              {
                point_id: point_id,
