...
export class OnboardingController {
  ...
  addStep = async (req: Request, res: Response) => {
    try {
      ...
      const result = await this.onboardingQuery.addStep(
        link_id,
        step_id,
        value,
      );
      res
        .status(201)
        .json({ message: "Step added successfully", data: result });
    } catch (err: any) {
      if (err.response) {
        ...
        return res
          .status(500)
          .send({ message: "Error adding Step", error: err });
      }
    }
  };
  ...
  getOnboarding = async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json({ message: "Onboarding retrieved successfully", data: result });
    } catch (err: any) {
      if (err.response) {
        return res.status(err.response.status).send({
          message: "Could not get Onboarding for member",
          error: err.response.data.err,
        });
      } else {
        return res.status(500).send({
          message: "Error getting Onboarding for a member",
  ...
  deleteOnboarding = async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json({ message: "Onboarding deleted successfully", data: result });
    } catch (err: any) {
      if (err.response) {
        return res.status(err.response.status).send({
          message: "Could not delete Onboarding",
          error: err.response.data.err,
        });
      } else {
        return res.status(500).send({
          message: "Error deleting Onboarding",
          error: JSON.stringify(err),
        });
      }
    }
