...
export class ProposalCountController {
  ...
  addSnapshotProposalCount = async (req: Request, res: Response) => {
    try {
      ...
      const result =
        await this.proposalCountQuery.addSnapshotProposalCount(proposalbody);
      res.status(201).json({
        message: "Snapshot Proposal Data created successfully",
        data: result,
      });
    } catch (err: any) {
      if (err.response) {
  ...
  getActiveProposalsCountforDao = async (req: Request, res: Response) => {
    try {
      ...
      const result: any =
        await this.proposalCountQuery.getActiveProposalsCountforDao(dao_id);
      res.status(200).json({ pending_proposal_count: result });
    } catch (err: any) {
      if (err.response) {
        return res.status(err.response.status).send({
          message: "Could not get Active Proposal for DAO",
          error: err.response.data.err,
        });
      } else {
        return res.status(500).send({
          message: "Error getting Active Proposal for a DAO",
