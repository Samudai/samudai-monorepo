...
export class ViewCountController {
  ...
  addDiscoveryViews = async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(201)
        .json({ message: "DiscoveryView added successfully", data: result });
    } catch (err: any) {
      if (err.response) {
        return res.status(err.response.status).send({
          message: "Could not add DiscoveryView",
          error: err.response.data.err,
        });
      } else {
        return res
          .status(500)
          .send({ message: "Error adding DiscoveryView", error: err });
      }
    }
  };
  ...
  getMostViewedDAO = async (req: Request, res: Response) => {
    try {
      ...
      res.status(200).json({
        message: "Most viewed DAO retrieved successfully",
        dao: mostViewedDAO![0],
      });
    } catch (err: any) {
      if (err.response) {
  ...
  getMostViewedContributor = async (req: Request, res: Response) => {
    try {
      ...
      res.status(200).json({
        message: "Most viewed Contributor retrieved successfully",
        contributor: mostViewedDAO![0],
      });
    } catch (err: any) {
      if (err.response) {
        return res.status(err.response.status).send({
