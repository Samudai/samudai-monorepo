...
export class MostActiveController {
  ...
  mostActiveDao = async (req: Request, res: Response) => {
    try {
      ...
      res.status(200).json({
        message: "Most active DAO retrieved successfully",
        dao: mostActiveDAO![0],
      });
    } catch (err: any) {
      if (err.response) {
  ...
  mostActiveContributor = async (req: Request, res: Response) => {
    try {
      const mostActiveContributor =
        await this.mostActiveQuery.mostActiveContributor();
      res.status(200).json({
        message: "Most active Contributor retrieved successfully",
        contributor: mostActiveContributor![0],
      });
    } catch (err: any) {
      if (err.response) {
        return res.status(err.response.status).send({
