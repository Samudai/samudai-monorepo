...
export class Routes {
  ...
  activityRouters = () => {
    ...
    this.activityRouter.get(
      "/dao/:daoId",
      this.activityController.getActivityByDAO,
    );
    this.activityRouter.get(
      "/action/:daoId/:action",
      this.activityController.getActivitybyAction,
    );
    this.activityRouter.get(
      "/member/:daoId/:memberId",
      this.activityController.getActivitybyMemberForDAO,
    );
    this.activityRouter.get(
      "/visibility/:daoId/:visibility",
      this.activityController.getActivityByVisibilityForDAO,
    );

    this.activityRouter.get(
      "/project/:projectId",
      this.activityController.getActivitybyProject,
    );
    this.activityRouter.get(
      "/task/:taskId",
      this.activityController.getActivitybyTask,
    );
    this.activityRouter.get(
      "/discussion/:discussionId",
      this.activityController.getActivityByDiscussion,
    );

    this.activityRouter.get(
      "/get/member/:memberId",
      this.activityController.getActivityByMember,
    );

    //Onboarding routes
    this.onboardingRouter.get(
      "/get/:linkId",
      this.onboardingController.getOnboarding,
    );
    this.onboardingRouter.post("/add", this.onboardingController.addStep);
    this.onboardingRouter.get(
      "/get/step/:stepId/:linkId",
      this.onboardingController.getOnboardingByStep,
    );
    this.onboardingRouter.delete(
      "/delete/:linkId",
      this.onboardingController.deleteOnboarding,
    );

    // Progress Bar
    this.progressBarRouter.post(
      "/dao/update",
      this.progressBarController.updateDAOProgressBar,
    );
    this.progressBarRouter.get(
      "/dao/:daoId",
      this.progressBarController.getDAOProgressBar,
    );
    this.progressBarRouter.post(
      "/contributor/update",
      this.progressBarController.updateContributorProgressBar,
    );
    this.progressBarRouter.get(
      "/contributor/:memberId",
      this.progressBarController.getContributorProgressBar,
    );

    // Most Active Routes
    this.mostActiveRouter.get("/dao", this.mostActiveController.mostActiveDao);
    this.mostActiveRouter.get(
      "/contributor",
      this.mostActiveController.mostActiveContributor,
    );

    // ViewCount Routes
    this.viewCountRouter.post(
      "/discovery/add",
      this.viewCountController.addDiscoveryViews,
    );
    this.viewCountRouter.get(
      "/mostviewed/dao",
      this.viewCountController.getMostViewedDAO,
    );
    this.viewCountRouter.get(
      "/mostviewed/contributor",
      this.viewCountController.getMostViewedContributor,
    );

    // Pending Proposal
    this.proposalCountRouter.post(
      "/snapshot/proposal/add",
      this.proposalCountController.addSnapshotProposalCount,
    );
    this.proposalCountRouter.get(
      "/get/activeproposal/:daoId",
      this.proposalCountController.getActiveProposalsCountforDao,
    );

    // Engagement Metrics
    // for most active
    this.engagementRouter.post(
      "/mostactive/add",
      this.engagementController.addMostActive,
    );
    this.engagementRouter.get(
      "/mostactive/dao",
      this.engagementController.getMostActiveDAO,
    );
    this.engagementRouter.get(
      "/mostactive/contributor",
      this.engagementController.getMostActiveContributor,
    );
    // for most viewed
    this.engagementRouter.post(
      "/mostviewed/add",
      this.engagementController.addMostViewed,
    );
    this.engagementRouter.get(
      "/mostviewed/dao",
      this.engagementController.getMostViewedDAO,
    );
    this.engagementRouter.get(
      "/mostviewed/contributor",
      this.engagementController.getMostViewedContributor,
    );

    // Feedback
    this.feedbackRouter.post(
      "/samudai/add",
      this.feedbackController.addFeedbackForSamudai,
    );

    // Admin
    this.adminRouter.post(
      "/dao/add",
      this.adminController.addAdminsForSamudaiDao,
    );
