...
export class PaymentController {
  ...
  addPayment = async (req: Request, res: Response) => {
    try {
      ...
      if (err.response) {
        ...
        return res
          .status(500)
          .send({ message: "Error creating Payment", error: err });
      }
    }
  };
  ...
  getPlatformPaymentsForDAO = async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json({ message: "Payments retrieved successfully", data: result });
    } catch (err: any) {
      if (err.response) {
        return res.status(err.response.status).send({
          message: "Could not get Payments for dao",
          error: err.response.data.err,
        });
      } else {
        return res.status(500).send({
          message: "Error getting Payments for a dao",
  ...
  getPlatformPaymentsByReceiver = async (req: Request, res: Response) => {
    try {
      ...
      const result: any =
        await this.paymentQuery.getPaymentByReceiver(receiver);
      res
        .status(200)
        .json({ message: "Payments retrieved successfully", data: result });
    } catch (err: any) {
      if (err.response) {
        return res.status(err.response.status).send({
          message: "Could not get Payments for receiver",
          error: err.response.data.err,
        });
      } else {
        return res.status(500).send({
          message: "Error getting Payments for a receiver",
          error: JSON.stringify(err),
        });
      }
    }
  ...
  getPlatformPaymentsByTaskId = async (req: Request, res: Response) => {
    try {
      ...
      if (err.response) {
        return res.status(err.response.status).send({
          message: "Could not get Payments for task",
          error: err.response.data.err,
        });
      } else {
        return res.status(500).send({
          message: "Error getting Payments for a task",
          error: JSON.stringify(err),
        });
      }
    }
  ...
  getPaymentById = async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json({ message: "Payment retrieved successfully", data: result });
    } catch (err: any) {
      if (err.response) {
        return res.status(err.response.status).send({
  ...
  updatePaymentStatus = async (req: Request, res: Response) => {
    try {
      ...
      console.log("payment_id", payment_id);
      const result: any = await this.paymentQuery.updatePaymentStatus(
        payment_id,
        status,
        completed_at,
        updated_by,
      ...
      res
        .status(200)
        .json({ message: "Payment status updated successfully", data: result });
    } catch (err: any) {
      if (err.response) {
        return res.status(err.response.status).send({
