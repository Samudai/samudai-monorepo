...
export class TokenGatingController {
  ...
  addTokenGating = async (req: Request, res: Response) => {
    try {
      ...
      const result = await this.tokenGatingQuery.addTokenGating(
        dao_id,
      ...
      res
        .status(201)
        .json({ message: "Token Gating creating Successfully", data: result });
    } catch (err: any) {
      if (err.response) {
        return res.status(err.response.status).send({
          message: "Could not create token gating",
          error: err.response.data.err,
        });
      } else {
        return res
          .status(500)
          .send({ message: "Error creating token gating", error: err });
      }
    }
  };
  ...
  getTokenGating = async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json({ message: "Token Gating fetched Successfully", data: result });
    } catch (err: any) {
      if (err.response) {
        return res.status(err.response.status).send({
          message: "Could not fetch token gating",
          error: err.response.data.err,
        });
      } else {
        return res
          .status(500)
          .send({ message: "Error fetching token gating", error: err });
      }
    }
  };
  ...
  deleteTokenGating = async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json({ message: "Token Gating deleted Successfully", data: result });
    } catch (err: any) {
      if (err.response) {
        return res.status(err.response.status).send({
          message: "Could not delete token gating",
          error: err.response.data.err,
        });
      } else {
        return res
          .status(500)
          .send({ message: "Error deleting token gating", error: err });
      }
    }
  };
}
