...
import (
	...
	"github.com/Samudai/backend/services/point/pkg/point"
	"github.com/Samudai/backend/shared/sqldb"
)

func CreateMemberDiscord(memberID string, discord point.MemberDiscord) error {
	db := sqldb.Point()
...
func UpdateMemberDiscord(memberID string, discord point.MemberDiscord) error {
	db := sqldb.Point()
...
func FetchDiscordForMember(memberID string) (point.MemberDiscord, error) {
	db := sqldb.Point()
	var discord point.MemberDiscord

	err := db.QueryRow(`SELECT member_id, discord_user_id, username, avatar,
	discriminator, public_flags, flags, banner,
	banner_color, accent_color, locale, mfa_enabled, verified, email
	FROM discord WHERE member_id = $1`, memberID).Scan(&discord.MemberID,
		&discord.DiscordID, &discord.Username, &discord.Avatar, &discord.Discriminator,
		&discord.PublicFlags, &discord.Flags, &discord.Banner, &discord.BannerColor,
		&discord.AccentColor, &discord.Locale, &discord.MfaEnabled, &discord.Verified,
		&discord.Email)
	if err != nil {
		return discord, err
	}
