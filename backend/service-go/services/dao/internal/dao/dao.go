...
func GetTrialDAO() (dao.DAOView, error) {
	db := sqldb.Dao()
	var DAO dao.DAOView
	var socialsJSON, tokensJSON, departmentsJSON, rolesJSON *json.RawMessage
	var pocMemberJSON, membersJSON, collaborationPassJSON, collaborationsJSON *json.RawMessage
	...
	err := db.QueryRow(`SELECT dao_id, name, about, guild_id, profile_picture,
		...
		FROM dao_view
		WHERE is_trial = true
		LIMIT 1`).Scan(&DAO.DAOID, &DAO.Name, &DAO.About, &DAO.GuildID, &DAO.ProfilePicture,
		&DAO.ContractAddress, &DAO.Snapshot, &DAO.OwnerID, &DAO.CreatedAt, &DAO.UpdatedAt,
		&rolesJSON, &departmentsJSON, &socialsJSON, &tokensJSON,
		&DAO.Onboarding, &DAO.TokenGating, &DAO.DAOType, pq.Array(&DAO.Tags), &DAO.OpenToCollaboration, &pocMemberJSON,
		&membersJSON, &DAO.MembersCount, &DAO.JoinDAOLink, &collaborationPassJSON, &collaborationsJSON,
	...
	if departmentsJSON != nil {
		if err = json.Unmarshal(*departmentsJSON, &DAO.Departments); err != nil {
			return DAO, err
		}
	}
	if rolesJSON != nil {
		if err = json.Unmarshal(*rolesJSON, &DAO.Roles); err != nil {
			return DAO, err
		}
	}
	if socialsJSON != nil {
		if err = json.Unmarshal(*socialsJSON, &DAO.Socials); err != nil {
			return DAO, err
		}
	}
	if tokensJSON != nil {
		if err = json.Unmarshal(*tokensJSON, &DAO.Tokens); err != nil {
			return DAO, err
		}
	}
	if pocMemberJSON != nil {
		if err = json.Unmarshal(*pocMemberJSON, &DAO.POCMember); err != nil {
			return DAO, err
		}
	}
	if membersJSON != nil {
		if err = json.Unmarshal(*membersJSON, &DAO.Members); err != nil {
			return DAO, err
		}
	}
	if collaborationPassJSON != nil {
		if err = json.Unmarshal(*collaborationPassJSON, &DAO.DaoCollaborationPass); err != nil {
			return DAO, err
		}
	}
	if collaborationsJSON != nil {
		if err = json.Unmarshal(*collaborationsJSON, &DAO.Collaborations); err != nil {
			return DAO, err
		}
	}
	if DaoSubscriptionResponseJSON != nil {
		if err = json.Unmarshal(*DaoSubscriptionResponseJSON, &DAO.Subscription); err != nil {
			return DAO, err
		}
	}
	return DAO, nil
