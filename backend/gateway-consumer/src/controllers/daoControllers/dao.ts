...
export class DAOProfileController {
    ...
    getTrialDAO = async (req: Request, res: Response) => {
        try {
            const result = await axios.get(`${process.env.SERVICE_DAO}/dao/trial`);
            return res.status(200).send({
                message: 'Trial DAO fetched successfully',
                data: result.data,
            });
        } catch (err: any) {
            if (err.response) {
                return res
                    .status(err.response.status)
                    .send({ message: 'Something went wrong while fetching trial dao', error: err.response.data });
            } else {
                return res.status(500).send({ message: 'Internal server error', error: JSON.stringify(err) });
            }
        }
