...
import {
  ...
  BountySubmissionNotificationMetaData,
  AccecptanceOfJobApplicantNotificationMetaData,
  postingJobNotificationMetaData,
  postingBountyNotificationMetaData,
} from './types';

export class JobNotificationTemplateHandler {
  postingJob = async (notificationPartialData: NotificationPartialData): Promise<WebNotification | null> => {
    try {
      const { to, from, metaData, timestamp } = notificationPartialData;
      const jwt = generateJWT(from.from);

      const openToWorkContributor = await axios.get(
        `${process.env.GATEWAY_URL}/api/member/get/all/contributors/open_to_work`,
        {
          headers: {
  ...
  postingBounty = async (notificationPartialData: NotificationPartialData): Promise<WebNotification | null> => {
    try {
      const { to, from, metaData, timestamp } = notificationPartialData;
      const jwt = generateJWT(from.from);
