...
const ProjectsBoard = () => {
    ...
    useEffect(() => {
        dispatch(changeProjectid({ projectid: id! }));
        ...
        if (
            ...
            setProject(val);
            setUsers(taskData?.contributors || []);
            if (showSettings) {
                settingsPopup.open();
            }
            dispatch(changeShowSettings({ showSettings: false }));
        } else if (!!activeDAO && projectSuccess) {
            dispatch(changeProjectid({ projectid: projectData.data?.project?.project_id! }));
            const val = { ...projectData?.data?.project };
    ...
    }, [daoid, projectData, taskData, showSettings, projectDao, activeDAO, id, taskForFormData]);
    ...
    return isLoading && loader1 ? (
        ...
        <div className={styles.root}>
                    ...
                    <div className={clsx('container', styles.container)}>
                        <Breadcrumbs
                            links={[
                                ...
                                { name: project?.title || '', href: location.pathname },
                                { name: 'Tasks', href: location.pathname },
                            ]}
                        />
                        <RouteHeader className={styles.header}>
            ...
            <TaskAdd close={handleTaskAddClose}/>
                    ...
                    <PopupBox
                        popupClassName={styles.settings_popup}
