...
import BoardCard from 'components/InvestmentBoard/elements/InvestmentBoardCard';
import InvestmentBoardCard from 'components/InvestmentBoard/elements/InvestmentBoardCard';
...
const InvestmentBoard: React.FC<BoardProps> = ({
  project,
  newTask,
  setNewTask,
  onUpdate,
  ...
  const [tasks, setTasks] = useState([] as dynamicColumnType[]);
  ...
  const [updateInvestmentColumn, { isLoading: loader3 }] =
    useUpdateInvestmentColumnsMutation();
  ...
  // useEffect(() => {
  //   // if (project?.tasks?.length && project?.tasks?.length > 0) {
  //   const tasks = KanbanHelper.getListItems(project);
  //   console.log('Kanban tasks', tasks, project);
  //   setTasks(tasks);
  //   // }
  // }, [project]);
  ...
  useEffect(() => {
    if (initialTasks ) {
      const tasks = KanbanHelper.getListItemsForInvestment(initialTasks, project);
      console.log('Kanban tasks', tasks, project, columns);
      setTasks(tasks);
    }
                        ...
                        {col.items
                          ...
                          .map((item, idx) => (
                            <InvestmentBoardCard
                              ...
                              onClick={() =>
                                handleClickDetails(
                                  item.task_id || item.mongo_object || ''
                                )
                              }
